...
func (g *generator) generateFile(file *descriptor.FileDescriptorProto) *plugin.CodeGeneratorResponse_File {
	g.P("# Code generated by protoc-gen-twirk_python ", gen.Version, ", DO NOT EDIT.")
	...
	g.P()
	g.P(`class twirkException(httplib.HTTPException):`)
	g.P(`    def __init__(self, code, message, meta):`)
	g.P(`        self.code = code`)
	g.P(`        self.message = message`)
	g.P(`        self.meta = meta`)
	g.P(`        super(twirkException, self).__init__(message)`)
	g.P()
	g.P(`    @classmethod`)
...
func (g *generator) generateProtobufClient(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	...
	g.P(`        req = Request(`)
	g.P(`            url=self.__target + "/twirk" + full_method,`)
	g.P(`            data=body,`)
	...
	g.P(`            raise twirkException.from_http_err(err)`)
	g.P(``)
