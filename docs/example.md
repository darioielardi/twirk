...
Let's make the canonical twirk example service: a `Haberdasher`.
...
package twirk.example.haberdasher;
...
```sh
$ protoc --proto_path=$GOPATH/src:. --twirk_out=. --go_out=. ./rpc/haberdasher/service.proto
```
...
/rpc
  /haberdasher
    ...
    service.twirk.go  # auto-generated by protoc-gen-twirk (servers, clients and interfaces)
```

If you open the generated `.twirk.go` file, you will see a Go interface like
this:
...
import (
    ...
    pb "github.com/darioielardi/twirkexample/rpc/haberdasher"
)
...
This meets the `Haberdasher` interface because it implements the `MakeHat` method, so we're ready to serve this over twirk!
...
This constructor wraps your interface implementation as an `twirkServer`, which
...
import (
    ...
    "github.com/darioielardi/twirkexample/internal/haberdasherserver"
    "github.com/darioielardi/twirkexample/rpc/haberdasher"
)
...
func main() {
  ...
  http.ListenAndServe(":8080", twirkHandler)
}
```
...
The JSONClient is included as reference, to show that a twirk server can handle
