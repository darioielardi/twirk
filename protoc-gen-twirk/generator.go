...
import (
	...
	"github.com/darioielardi/twirk/internal/gen"
	"github.com/darioielardi/twirk/internal/gen/stringutils"
	"github.com/darioielardi/twirk/internal/gen/typemap"
)
...
func (t *twirk) Generate(in *plugin.CodeGeneratorRequest) *plugin.CodeGeneratorResponse {
	params, err := parseCommandLineParams(in.GetParameter())
	if err != nil {
		gen.Fail("could not parse parameters passed to --twirk_out", err.Error())
	}
...
func (t *twirk) registerPackageName(name string) (alias string) {
	alias = name
	i := 1
...
func (t *twirk) generate(file *descriptor.FileDescriptorProto) *plugin.CodeGeneratorResponse_File {
...
func (t *twirk) generateFileHeader(file *descriptor.FileDescriptorProto) {
	t.P("// Code generated by protoc-gen-twirk ", gen.Version, ", DO NOT EDIT.")
	t.P("// source: ", file.GetName())
	t.P()
	if t.filesHandled == 0 {
		t.P("/*")
		t.P("Package ", t.genPkgName, " is a generated twirk stub package.")
		t.P("This code was generated with github.com/darioielardi/twirk/protoc-gen-twirk ", gen.Version, ".")
		t.P()
...
func (t *twirk) generateImports(file *descriptor.FileDescriptorProto) {
	if len(file.Service) == 0 {
		return
	}
	...
	t.P(`import `, t.pkgs["twirk"], ` "github.com/darioielardi/twirk"`)
	t.P(`import `, t.pkgs["ctxsetters"], ` "github.com/darioielardi/twirk/ctxsetters"`)
	t.P()
...
func (t *twirk) generateUtilImports() {
...
// Generate utility functions used in twirk code.
// These should be generated just once per package.
func (t *twirk) generateUtils() {
	...
	t.P(`}`)
	t.P()
	t.P(`// twirkServer is the interface generated server structs will support: they're`)
	...
	t.P(`// Most people can think of twirkServers as just http.Handlers.`)
	t.P(`type twirkServer interface {`)
	...
	t.P(`	// ProtocGentwirkVersion is the semantic version string of the version of`)
	t.P(`	// twirk used to generate this file.`)
	t.P(`	ProtocGentwirkVersion() string`)
	t.P(`// PathPrefix returns the HTTP URL path prefix for all methods handled by this`)
	t.P(`// service. This can be used with an HTTP mux to route twirk requests`)
	t.P(`// alongside non-twirk requests on one HTTP listener.`)
	t.P(`	PathPrefix() string`)
	t.P(`}`)
	t.P()

	t.P(`// WriteError writes an HTTP response with a valid twirk error format (code, msg, meta).`)
	t.P(`// Useful outside of the twirk server (e.g. http middleware), but does not trigger hooks.`)
	t.P(`// If err is not a twirk.Error, it will get wrapped with twirk.InternalErrorWith(err)`)
	t.P(`func WriteError(resp `, t.pkgs["http"], `.ResponseWriter, err error) {`)
	t.P(`  writeError(`, t.pkgs["context"], `.Background(), resp, err, nil)`)
	t.P(`}`)
	t.P()

	t.P(`// writeError writes twirk errors in the response and triggers hooks.`)
	t.P(`func writeError(ctx `, t.pkgs["context"], `.Context, resp `, t.pkgs["http"], `.ResponseWriter, err error, hooks *`, t.pkgs["twirk"], `.ServerHooks) {`)
	t.P(`  // Non-twirk errors are wrapped as Internal (default)`)
	t.P(`  twerr, ok := err.(`, t.pkgs["twirk"], `.Error)`)
	t.P(`  if !ok {`)
	t.P(`    twerr = `, t.pkgs["twirk"], `.InternalErrorWith(err)`)
	t.P(`  }`)
	t.P(``)
	t.P(`  statusCode := `, t.pkgs["twirk"], `.ServerHTTPStatusFromErrorCode(twerr.Code())`)
	...
	t.P(`// a context through the twirk.WithHTTPRequestHeaders function.`)
	...
	t.P(`  header, ok := `, t.pkgs["twirk"], `.HTTPRequestHeaders(ctx)`)
	...
	t.P(`  req.Header.Set("twirk-Version", "`, gen.Version, `")`)
	t.P(`  return req, nil`)
	t.P(`}`)
	t.P()
	...
	t.P(`}`)
	t.P()
	t.P(`// marshalErrorToJSON returns JSON from a twirk.Error, that can be used as HTTP error response body.`)
	t.P(`// If serialization fails, it will use a descriptive Internal error instead.`)
	t.P(`func marshalErrorToJSON(twerr `, t.pkgs["twirk"], `.Error) []byte {`)
	...
	t.P(`  if err != nil {`)
	t.P(`    buf = []byte("{\"type\": \"" + `, t.pkgs["twirk"], `.Internal +"\", \"msg\": \"There was an error but it could not be serialized into JSON\"}") // fallback`)
	t.P(`  }`)
	t.P(``)
	t.P(`  return buf`)
	t.P(`}`)
	...
	t.P(`// errorFromResponse builds a twirk.Error from a non-200 HTTP response.`)
	t.P(`// If the response has a valid serialized twirk error, then it's returned.`)
	t.P(`// If not, the response status code is used to generate a similar twirk`)
	t.P(`// error. See twirkErrorFromIntermediary for more info on intermediary errors.`)
	t.P(`func errorFromResponse(resp *`, t.pkgs["http"], `.Response) `, t.pkgs["twirk"], `.Error {`)
	...
	t.P(`    // twirk clients don't follow redirects automatically, twirk only handles`)
	...
	t.P(`    return twirkErrorFromIntermediary(statusCode, msg, location)`)
	t.P(`  }`)
	t.P(``)
	...
	t.P(`    return twirkErrorFromIntermediary(statusCode, msg, string(respBodyBytes))`)
	t.P(`  }`)
	t.P(``)
	t.P(`  errorCode := `, t.pkgs["twirk"], `.ErrorCode(tj.Code)`)
	t.P(`  if !`, t.pkgs["twirk"], `.IsValidErrorCode(errorCode) {`)
	t.P(`    msg := "invalid type returned from server error response: "+tj.Code`)
	t.P(`    return `, t.pkgs["twirk"], `.InternalError(msg)`)
	t.P(`  }`)
	t.P(``)
	t.P(`  twerr := `, t.pkgs["twirk"], `.NewError(errorCode, tj.Msg)`)
	...
	t.P(`// twirkErrorFromIntermediary maps HTTP errors from non-twirk sources to twirk errors.`)
	t.P(`// The mapping is similar to gRPC: https://github.com/grpc/grpc/blob/master/doc/http-grpc-status-mapping.md.`)
	t.P(`// Returned twirk Errors have some additional metadata for inspection.`)
	t.P(`func twirkErrorFromIntermediary(status int, msg string, bodyOrLocation string) `, t.pkgs["twirk"], `.Error {`)
	t.P(`  var code `, t.pkgs["twirk"], `.ErrorCode`)
	t.P(`  if isHTTPRedirect(status) { // 3xx`)
	t.P(`    code = `, t.pkgs["twirk"], `.Internal`)
	t.P(`  } else {`)
	t.P(`    switch status {`)
	t.P(`    case 400: // Bad Request`)
	t.P(`      code = `, t.pkgs["twirk"], `.Internal`)
	t.P(`    case 401: // Unauthorized`)
	t.P(`      code = `, t.pkgs["twirk"], `.Unauthenticated`)
	t.P(`    case 403: // Forbidden`)
	t.P(`      code = `, t.pkgs["twirk"], `.PermissionDenied`)
	t.P(`    case 404: // Not Found`)
	t.P(`      code = `, t.pkgs["twirk"], `.BadRoute`)
	t.P(`    case 429, 502, 503, 504: // Too Many Requests, Bad Gateway, Service Unavailable, Gateway Timeout`)
	t.P(`      code = `, t.pkgs["twirk"], `.Unavailable`)
	t.P(`    default: // All other codes`)
	t.P(`      code = `, t.pkgs["twirk"], `.Unknown`)
	t.P(`    }`)
	t.P(`  }`)
	t.P(``)
	t.P(`  twerr := `, t.pkgs["twirk"], `.NewError(code, msg)`)
	...
	t.P(`func wrapInternal(err error, prefix string) `, t.pkgs["twirk"], `.Error {`)
	t.P(`	return `, t.pkgs["twirk"], `.InternalErrorWith(&wrappedError{prefix: prefix, cause: err})`)
	t.P(`}`)
	...
	t.P(`// ensurePanicResponses makes sure that rpc methods causing a panic still result in a twirk Internal`)
	...
	t.P(`func ensurePanicResponses(ctx `, t.pkgs["context"], `.Context, resp `, t.pkgs["http"], `.ResponseWriter, hooks *`, t.pkgs["twirk"], `.ServerHooks) {`)
	...
	t.P(`}`)
	t.P(``)
	t.P(`// internalWithCause is a twirk Internal error wrapping an original error cause, accessible`)
	...
	t.P(`func (e *internalWithCause) Code() `, t.pkgs["twirk"], `.ErrorCode { return `, t.pkgs["twirk"], `.Internal }`)
	...
	t.P(`func (e *internalWithCause) WithMeta(key string, val string) `, t.pkgs["twirk"], `.Error { return e }`)
	t.P()

	t.P(`// malformedRequestError is used when the twirk server cannot unmarshal a request`)
	t.P(`func malformedRequestError(msg string) `, t.pkgs["twirk"], `.Error {`)
	t.P(`	return `, t.pkgs["twirk"], `.NewError(`, t.pkgs["twirk"], `.Malformed, msg)`)
	t.P(`}`)
	t.P()

	t.P(`// badRouteError is used when the twirk server cannot route a request`)
	t.P(`func badRouteError(msg string, method, url string) `, t.pkgs["twirk"], `.Error {`)
	t.P(`	err := `, t.pkgs["twirk"], `.NewError(`, t.pkgs["twirk"], `.BadRoute, msg)`)
	t.P(`	err = err.WithMeta("twirk_invalid_route", method+" "+url)`)
	t.P(`	return err`)
	t.P(`}`)
	t.P()
	...
	t.P(`// doProtobufRequest makes a Protobuf request to the remote twirk service.`)
	...
	t.P(`// doJSONRequest makes a JSON request to the remote twirk service.`)
	...
	t.P(`// Call twirk.ServerHooks.RequestReceived if the hook is available`)
	t.P(`func callRequestReceived(ctx `, t.pkgs["context"], `.Context, h *`, t.pkgs["twirk"], `.ServerHooks) (`, t.pkgs["context"], `.Context, error) {`)
	...
	t.P(`}`)
	t.P()
	t.P(`// Call twirk.ServerHooks.RequestRouted if the hook is available`)
	t.P(`func callRequestRouted(ctx `, t.pkgs["context"], `.Context, h *`, t.pkgs["twirk"], `.ServerHooks) (`, t.pkgs["context"], `.Context, error) {`)
	...
	t.P(`}`)
	t.P()
	t.P(`// Call twirk.ServerHooks.RequestDecoded if the hook is available`)
	t.P(`func callRequestDecoded(ctx `, t.pkgs["context"], `.Context, req interface{}, h *`, t.pkgs["twirk"], `.ServerHooks) (`, t.pkgs["context"], `.Context, error) {`)
	t.P(`  if h == nil || h.RequestDecoded == nil {`)
	t.P(`    return ctx, nil`)
	t.P(`  }`)
	t.P(`  return h.RequestDecoded(ctx, req)`)
	t.P(`}`)
	t.P()
	t.P(`// Call twirk.ServerHooks.ResponsePrepared if the hook is available`)
	t.P(`func callResponsePrepared(ctx `, t.pkgs["context"], `.Context, h *`, t.pkgs["twirk"], `.ServerHooks) `, t.pkgs["context"], `.Context {`)
	...
	t.P(`}`)
	t.P()
	t.P(`// Call twirk.ServerHooks.ResponseSent if the hook is available`)
	t.P(`func callResponseSent(ctx `, t.pkgs["context"], `.Context, h *`, t.pkgs["twirk"], `.ServerHooks) {`)
	...
	t.P(`}`)
	t.P()
	t.P(`// Call twirk.ServerHooks.Error if the hook is available`)
	t.P(`func callError(ctx `, t.pkgs["context"], `.Context, h *`, t.pkgs["twirk"], `.ServerHooks, err `, t.pkgs["twirk"], `.Error) `, t.pkgs["context"], `.Context {`)
...
func (t *twirk) sectionComment(sectionTitle string) {
	t.P()
...
func (t *twirk) generateService(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto, index int) {
	servName := serviceName(service)

	t.sectionComment(servName + ` Interface`)
	t.generatetwirkInterface(file, service)
...
func (t *twirk) generatetwirkInterface(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	servName := serviceName(service)
...
func (t *twirk) generateSignature(method *descriptor.MethodDescriptorProto) string {
...
func (t *twirk) generateClient(name string, file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	servName := serviceName(service)
...
func (t *twirk) generateServer(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	servName := serviceName(service)
	...
	t.P(`  `, servName)
	t.P(`  hooks     *`, t.pkgs["twirk"], `.ServerHooks`)
	t.P(`}`)
	t.P()

	// Constructor for server implementation
	t.P(`func New`, servName, `Server(svc `, servName, `, hooks *`, t.pkgs["twirk"], `.ServerHooks) twirkServer {`)
	...
	// Write Errors
	t.P(`// writeError writes an HTTP response with a valid twirk error format, and triggers hooks.`)
	t.P(`// If err is not a twirk.Error, it will get wrapped with twirk.InternalErrorWith(err)`)
...
func (t *twirk) generateServerRouting(servStruct string, file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	pkgName := pkgName(file)
	servName := serviceName(service)

	pathPrefixConst := servName + "PathPrefix"
	t.P(`// `, pathPrefixConst, ` is used for all URL paths on a twirk `, servName, ` server.`)
	t.P(`// Requests are always: POST `, pathPrefixConst, `/method`)
	t.P(`// It can be used in an HTTP mux to route twirk requests along with non-twirk requests on other routes.`)
...
func (t *twirk) generateServerMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
...
func (t *twirk) generateServerJSONMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	servStruct := serviceStruct(service)
	methName := stringutils.CamelCase(method.GetName())
	servName := serviceName(service)
	...
	t.P(`  }`)
	t.P()
	t.P(`  ctx, err = callRequestDecoded(ctx, reqContent, s.hooks)`)
	t.P(`  if err != nil {`)
	t.P(`    s.writeError(ctx, resp, err)`)
	t.P(`    return`)
	t.P(`  }`)
	...
	t.P(`    s.writeError(ctx, resp, `, t.pkgs["twirk"], `.InternalError("received a nil *`, t.goTypeName(method.GetOutputType()), ` and nil error while calling `, methName, `. nil responses are not supported"))`)
	t.P(`    return`)
	t.P(`  }`)
	t.P()
	...
	t.P(`    twerr := `, t.pkgs["twirk"], `.NewError(`, t.pkgs["twirk"], `.Unknown, msg)`)
...
func (t *twirk) generateServerProtobufMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	servStruct := serviceStruct(service)
	methName := stringutils.CamelCase(method.GetName())
	servName := serviceName(service)
...
func (t *twirk) serviceMetadataVarName() string {
	return fmt.Sprintf("twirkFileDescriptor%d", t.filesHandled)
}

func (t *twirk) generateServiceMetadataAccessors(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	servStruct := serviceStruct(service)
	index := 0
	...
	t.P(`}`)
	t.P()
	t.P(`func (s *`, servStruct, `) ProtocGentwirkVersion() (string) {`)
...
func (t *twirk) generateFileDescriptor(file *descriptor.FileDescriptorProto) {
...
func (t *twirk) printComments(comments typemap.DefinitionComments) bool {
...
func (t *twirk) goTypeName(protoName string) string {
...
func (t *twirk) goPackageName(file *descriptor.FileDescriptorProto) string {
	return t.fileToGoPackageName[file]
}

func (t *twirk) formattedOutput() string {
